...
impl Prover {
    ...
    pub fn create_credential_request(&self, cred_def: &CredentialDefinition, offer: &CredentialOffer) -> anyhow::Result<(CredentialRequest, CredentialRequestMetadata)> {
        Ok(prover::create_credential_request(Some("entropy"), None, cred_def, &self.link_secret, "my-secret", offer)?)
    }

    pub fn process_credential(&mut self, mut credential: Credential, metadata: &CredentialRequestMetadata, cred_def: &CredentialDefinition) -> anyhow::Result<()> {
        prover::process_credential(&mut credential, metadata, &self.link_secret, cred_def, None)?;
        self.credentials.push(credential);
        Ok(())
    }

    pub fn create_presentation(&self, pres_req: &PresentationRequest, schemas: &HashMap<SchemaId, Schema>, cred_defs: &HashMap<CredentialDefinitionId, CredentialDefinition>) -> anyhow::Result<Presentation> {
